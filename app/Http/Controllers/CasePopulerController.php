...
class CasePopulerController extends Controller
{
    public function index() {
        if(Session::get('login')){
            if(request()->ajax()) {
                return Datatables::of(CasePopuler::all())
                ->addColumn('gambar', function($case){
                    $url=$case->gambar;
                    return '<img src="'.$url.'" border="0" width="100" class="img-rounded" align="center" />';
                })
                ->addColumn('action', 'kelola_case_populer/action_button')
                ->rawColumns(['gambar','action'])
                ->addIndexColumn()
                ->make(true);
            }
            return view('kelola_case_populer/index');
        }
        else{
    ...
    {
        $where = array('id' => $id);
        $user  = CasePopuler::where($where)->first();
    ...
    public function update(Request $request)
    {
        ...
        if ($images != '') {
            ...
            $data2 = DB::table('casestudy_populer')
            ->where('id','=', $request->id)
            ->first();
        ...
        CasePopuler::where('id',$request->id)->update($form_data);
